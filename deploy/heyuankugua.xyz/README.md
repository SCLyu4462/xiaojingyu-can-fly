...
如果手机上打开发现画面右边或底部空出一条，按这个顺序查：

1. **先确认不是缓存**：这个 html 没有缓存头，但手机浏览器仍可能缓存。
   手机浏览器里打开 `https://heyuankugua.xyz/xiaojingyu/index.html?v=2`，
   或清一下站点数据、用无痕窗口再试。
2. **打开调试层看数字**：访问 `https://heyuankugua.xyz/xiaojingyu/index.html?debug=1`
   底部会出现一行黑底文字，长这样：

   ```
   视口 innerW/H 393x851   文档 393x851
   可视 visualViewport 393x851 scale=1
   采用(三者取最小) 393x851   DPR 2
   画布盒子 393x851  缓冲 786x1702
   scale 0.9357  offX 0.0  offY 88.6  可见逻辑 420x909
   ```

   判读方法：
   * 「画布盒子」必须等于「可视 visualViewport」——不等就是适配还有问题，把这行发我
   * 「可见逻辑」必须 ≥ `420x720`——小于说明玩法区被裁了
   * `scale` 明显不等于 1（比如 0.5、2）说明页面被缩放了

3. **这一版已经修的问题**：早期版本用「布局视口」测量尺寸，在移动端地址栏收放、
   双指缩放、或首屏布局未稳定时会算小，导致画布比可视区域窄一条。现在改成
   **三路视口取最小值（视觉视口 > 窗口 > 文档元素）**，画布改为 `position: fixed`
   钉在视口上，并监听 `resize` / `orientationchange` / `visualViewport` /
   `ResizeObserver` / `pageshow`，启动后再补两次校准。
4. 若第 2 步的数字正常但画面仍不对，把那张带调试层的截图发我即可。
